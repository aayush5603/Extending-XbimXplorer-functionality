...
public class DamageData
{
	...
	public string Hüllrohr_damage { get; set; }
	public string Spannstahl_damage { get; set; }
	public string CombinedName => $"{Feld} {VBA}";
}
...
namespace XbimXplorer
{
    ...
    public partial class XplorerMainWindow : IXbimXplorerPluginMasterWindow, INotifyPropertyChanged
    {
		...
		private List<DamageData> _damageDataList = new List<DamageData>();
		...
		private void LoadDatabase_Click(object sender, RoutedEventArgs e)
		{
			...
			if (openFileDialog.ShowDialog() == true)
			{
				...
				LoadDamageDataFromDatabase(dbFilePath);
				// MessageBox.Show("Database Loaded successfully");
			}
		}

		private void LoadDamageDataFromDatabase(string dbFilePath)
		{
			try
			{
				using (var connection = new SqliteConnection($"Data Source={dbFilePath}"))
				{
					...
					// SQL query to read data (we can change table and column names as per our database schema)
					var command = connection.CreateCommand();
					command.CommandText = "SELECT Feld, VBA, total_Hüllrohr_damage, total_Spannstahl_damage FROM section_damage"; 

					using (var reader = command.ExecuteReader())
					{
						_damageDataList.Clear(); // Clear previous data if any

						while (reader.Read())
						{
							var damageData = new DamageData
							{
								Feld = !reader.IsDBNull(0) ? reader.GetString(0) : "Unknown", // Replace with the correct column index or name
								VBA = !reader.IsDBNull(1) ? reader.GetString(1) : "Unknown",
								Hüllrohr_damage = !reader.IsDBNull(2) ? reader.GetString(2) : "Unknown",
								Spannstahl_damage = !reader.IsDBNull(3) ? reader.GetString(3) : "Unknown",
								
							};

							// Add to List
							_damageDataList.Add(damageData);
						}
					}
				}

				// Log data to debug output if needed
				/*foreach (var entry in _damageDataList)
				{
					System.Diagnostics.Debug.WriteLine($"{entry.Feld}: {entry.Hüllrohr_damage}, {entry.Spannstahl_damage}");
				}*/
...
var selectedEntity = SelectedItem;
if (selectedEntity is IIfcObject ifcRoot)
{
// convert the element name to string and trim any whitespace
string elementName = ifcRoot.Name?.ToString().Trim() ?? "Unknown";

// Get the parent object (Cantilever)
var relationship = ifcRoot.Decomposes.FirstOrDefault();
if (relationship != null && relationship.RelatingObject is IIfcObject cantilever)
{
	//convert the cantilever name to string and trim any whitespace
	string cantileverName = cantilever.Name?.ToString().Trim() ?? "Unknown";

	//had to add the below method as there are a few names in the IFC model whose section names are 1.001,3.001 and not 1,3.
	// tried to change these names using Blender but did not work.


	// Extract VBA part and remove the decimal part (e.g., "5.001" to "5")
	string vbaPart = elementName.Split(' ').Last();  // Get the last part of the name
	if (vbaPart.Contains("."))
	{
		vbaPart = vbaPart.Split('.')[0];  // Keep only the integer part
	}

	// Combine Feld and VBA for matching
	string combinedName = $"{cantileverName} {vbaPart}";

	// Debugging: Show the combined name being checked(checking for whitespace or any hidden characters)
	// MessageBox.Show($"Checking damage data for: {combinedName}");

	// Check if the combined name exists in the damage data
	var matchedDamage = _damageDataList.FirstOrDefault(d =>
		d.Feld.Equals(cantileverName, StringComparison.OrdinalIgnoreCase) &&
		d.VBA.Equals(vbaPart, StringComparison.OrdinalIgnoreCase));

	if (matchedDamage != null)
	{
		// Show damage data if found
		MessageBox.Show($"Damage Data Found:\n\nFeld: {matchedDamage.Feld}\nVBA: {matchedDamage.VBA}\nHüllrohr Damage: {matchedDamage.Hüllrohr_damage}\nSpannstahl Damage: {matchedDamage.Spannstahl_damage}",
						"Damage Information", MessageBoxButton.OK, MessageBoxImage.Information);
	}
	else
	{
		// No data found
		MessageBox.Show($"No damage data found for {combinedName}", "No Data", MessageBoxButton.OK, MessageBoxImage.Warning);
	}
}
else
{
	MessageBox.Show("The selected entity does not have a parent object.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
}
}
else
{
MessageBox.Show("The selected entity does not have a name property.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
}
}
